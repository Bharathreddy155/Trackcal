    ...
    <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-extrabold text-white uppercase tracking-wider">Today's Supplements</h3>
        </div>
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-sm font-bold text-white">LeanFit Whey Protein</h4>
                <p className="text-xs text-slate-400 font-mono">1 scoop (35g) • 140 kcal • 24g P • 4.8g BCAAs</p>
              </div>
            </div>

            {wheyState.taken || wheyInSnackCounted ? (
              <span className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-cyan-950 text-cyan-300 border border-cyan-500/30">
                <CheckCircle2 className="w-3.5 h-3.5" /> Taken
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-slate-800 text-slate-400">
          ...
          {wheyInSnackCounted && (
            <div className="mt-3 p-2 bg-slate-950/80 border border-cyan-500/20 rounded-lg flex items-center gap-2 text-xs text-cyan-300">
          ...
          <div className="mt-3 flex items-center justify-between pt-2 border-t border-slate-800/60">
            <div className="flex items-center gap-1">
              <span className="text-xs text-slate-400 mr-1 font-semibold">Scoops:</span>
              {[0.5, 1, 1.5].map((sc) => (
                <button
                  key={sc}
            ...
            {wheyState.taken ? (
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-cyan-500 text-slate-950 font-bold text-xs hover:brightness-110 transition shadow"
              >
                Log Whey
              </button>
            )}
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <span className="text-xl">⚡</span>
              <div>
                <h4 className="text-sm font-bold text-white">Creatine Monohydrate</h4>
                <p className="text-xs text-slate-400 font-mono">Daily Target: 3 g • 0 kcal</p>
              </div>
            </div>

            {creatineState.taken ? (
              <span className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-950 text-emerald-300 border border-emerald-500/30">
                <CheckCircle2 className="w-3.5 h-3.5" /> Taken
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-slate-800 text-slate-400">
          ...
          <div className="mt-3 flex items-center justify-between pt-2 border-t border-slate-800/60">
            <span className="text-xs text-slate-400 font-mono">3g / day boost</span>
            ...
            {creatineState.taken ? (
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-emerald-500 text-slate-950 font-bold text-xs hover:brightness-110 transition shadow"
              >
                Log Creatine (3g)
              </button>
            )}
