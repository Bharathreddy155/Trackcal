    ...
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredFoods.length === 0 ? (
          <div className="col-span-full text-center py-12 text-slate-400 dark:text-slate-500 text-sm glass-panel rounded-2xl">
          ...
          filteredFoods.map((food) => (
            <div
              key={food.id}
              className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-extrabold text-slate-800 dark:text-white text-base">{food.name}</h3>
                    {food.brand && <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">{food.brand}</p>}
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between text-xs font-mono bg-slate-50 dark:bg-slate-950/80 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
                  <span className="text-slate-500 dark:text-slate-400">Serving:</span>
                  <span className="font-bold text-slate-800 dark:text-slate-200">{food.servingSize} {food.servingUnit}</span>
                </div>

                {/* Macro Breakdown */}
                <div className="grid grid-cols-5 gap-1 text-center mt-3 p-2 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-mono">
                  <div>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-sans">kcal</span>
                    <span className="font-bold text-slate-900 dark:text-white">{food.calories}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-sans">Prot</span>
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">{food.protein}g</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-sans">Carb</span>
                    <span className="font-bold text-cyan-600 dark:text-cyan-400">{food.carbs}g</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-sans">Fat</span>
                    <span className="font-bold text-amber-600 dark:text-amber-400">{food.fat}g</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-sans">Fib</span>
                    <span className="font-bold text-violet-600 dark:text-violet-400">{food.fiber}g</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800/60 flex items-center justify-between">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                  food.isCustom
                    ? 'bg-purple-50 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-500/30'
                ...
                {food.isCustom && (
                  <button
                    ...
                    className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 text-xs flex items-center gap-1 font-medium transition"
                  >
